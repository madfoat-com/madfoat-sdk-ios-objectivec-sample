...
public protocol MadFoatControllerDelegate {
...
public class MadFoatController: UIViewController, XMLParserDelegate {

    @objc var webView : WKWebView = WKWebView()
    
    var actInd: UIActivityIndicatorView?
    
    public var delegate : MadFoatControllerDelegate?
...
//                url = URL(string:"https://uat-secure.telrdev.com/gateway/mobile.xml")
        ...
        //url = URL(string:"https://secure.telr.com/gateway/ql/SPARKDIGITALCOMPLEXSOLE_359837.html")
            if let newurl = url{
                    ...
                    if let data = data{
                        ...
                        if let message = xmlresponse["mobile","auth","message"].text{
                            ...
                            if(message == "Authorised"){
                                ...
                                let telrResponseModel = MadFoatResponseModel()
...
extension MadFoatController : WKNavigationDelegate, WKUIDelegate, UIScrollViewDelegate{
    ...
    func saveTheCard(card:MadFoatResponseModel) {
       
        if let data = UserDefaults.standard.data(forKey: "cards") {
            do {
                let decoder = JSONDecoder()

                var telrResponseModels = try decoder.decode([MadFoatResponseModel].self, from: data)
