...
public class MadFoatResponseModel : Encodable & Decodable{
    ...
    public func getSavedCards() -> [MadFoatResponseModel] {
        
        if let data = UserDefaults.standard.data(forKey: "cards") {
            do {
                let decoder = JSONDecoder()

                let telrResponseModels = try decoder.decode([MadFoatResponseModel].self, from: data)
